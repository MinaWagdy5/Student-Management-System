...
class Ui_MainWindow(object):
    ...
    def search_class(self):
                ...
                # Filter data for the given class name
                grouped = df[df['course_name'] == class_name]
                print(grouped)
                print(class_df)
                df_string = class_df.to_string()
                self.class_info.setText(df_string)

                # Display the result in the QTableWidget
                self.display_results(grouped)

    def display_results(self, data):
        # Clear previous contents of the table
        self.class_search_result.clearContents()
        self.class_search_result.setRowCount(0)

        # Set the table headers
        headers = list(data.columns)
        self.class_search_result.setColumnCount(len(headers))
        self.class_search_result.setHorizontalHeaderLabels(headers)
        

        # Populate the table with data
        for row_index, row_data in data.iterrows():
                # Adjust row index by 1 if necessary (assuming indexing starts from 1)
                row_index = self.class_search_result.rowCount()
                self.class_search_result.insertRow(row_index)
                for col_index, value in enumerate(row_data):
                        item = QTableWidgetItem(str(value))
                # Adjust column index by 1 if necessary (assuming indexing starts from 1)
                        self.class_search_result.setItem(row_index, col_index, item)
...
"}")
        ...
        self.class_info = QtWidgets.QLabel(self.frame_20)
        self.class_info.setObjectName("class_info")
        self.horizontalLayout_36.addWidget(self.class_info)
...
" /* SEE THE EFFECT BELOW*/")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.class_info.setText(_translate("MainWindow", "TextLabel"))
